...
class KouseiGaussianModel:
    ...
    # def create_from_pcd_mixed(self, pcd : BasicPointCloud, spatial_lr_scale : float,fg=None):
    ...
    def create_from_pcd_mixed(self, bg : BasicPointCloud, spatial_lr_scale : float,fg=None):
        self.spatial_lr_scale = spatial_lr_scale
        self.bg_num = bg.points.shape[0]
        pcd_points = np.concatenate((bg.points,fg.points),axis=0)
        fused_point_cloud = torch.tensor(pcd_points).float().cuda()
        pcd_colors = np.concatenate((bg.colors,fg.colors),axis=0)
        fused_color = RGB2SH(torch.tensor(pcd_colors).float().cuda())
        features = torch.zeros((fused_color.shape[0], 3, (self.max_sh_degree + 1) ** 2)).float().cuda()
        features[:, :3, 0 ] = fused_color
        features[:, 3:, 1:] = 0.0
        print("Number of points at initialisation bg: ", bg.points.shape[0])
        print("Number of points at initialisation fg: ", fg.points.shape[0])
    ...
    def save_ply(self, path):
        ...
        xyz = self._xyz.detach().cpu().numpy() #0625问题 很大 4359330
